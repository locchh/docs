...
The DXGW removes the same-Region restriction of a private VIF, but it is not a global transit router. Two VPCs whose VGWs associate with one DXGW cannot normally use it to exchange traffic with each other. One documented exception, implemented in November 2021, matters because it is a silent connectivity leak: if on premises advertises a supernet that covers both VPC CIDRs, such as 10.0.0.0/8 over VPCs at 10.0.0.0/24 and 10.0.1.0/24, across both VGW associations on the same private VIF, those VPCs can reach each other through the Direct Connect endpoint. To keep them separated, advertise specific non-overlapping prefixes rather than a supernet, or give each VPC its own DXGW. The equivalent control for Transit Gateway associations is a blackhole route on the VPC attachments. Likewise, a DXGW association does not make VPC CIDRs transitive through an on-premises router unless the route advertisements and security policy deliberately support that path. Use Transit Gateway peering or Cloud WAN when VPC-to-VPC connectivity is the requirement.
...
Current AWS documentation also defines large-bandwidth VPN tunnels, which reach up to 5 Gbps per tunnel. That figure belongs to the tunnel itself and must not be attributed to the **Site-to-Site VPN Concentrator**, which is a different resource solving a different problem. The Concentrator aggregates 25 or more remote sites that each need under 100 Mbps: it caps an individual site at 100 Mbps, twelve times lower than a standard tunnel, and the whole attachment at 5 Gbps. It attaches only to a Transit Gateway, not to a virtual private gateway or Cloud WAN, and supports BGP only. Read it as fan-in for many small branches, never as a way to make one tunnel faster. The SAA-C03 and SAP-C02 guides predate both capabilities, so an exam question that contrasts one standard VPN with several connections still expects the established bandwidth-scaling rule unless it explicitly names the newer tunnel type. Architecturally, do not claim that every VPN tunnel is permanently capped at 1.25 Gbps.
...
**AWS Network Manager** is the inventory, visualization and monitoring layer over this topology, not a router. Its global network is a container holding registered Transit Gateways beside a logical model of the on-premises estate: sites, the devices at them and the links connecting those devices. Registering gateways produces one topology view across Regions and accounts and one place to receive topology and BGP state change events, delivered through Amazon EventBridge so they can drive an alarm or a ticket. Route Analyzer traces a path between two endpoints against the route tables as configured, reports where forwarding fails and shows whether the return path is asymmetric, which replaces reading several route tables by hand during an incident.

What Network Manager does not do is change forwarding. It observes and reports, so a requirement to unify routing policy across Regions is a Cloud WAN answer, while a requirement to see, audit or troubleshoot a Transit Gateway topology that already exists is a Network Manager answer. Registering every gateway in Network Manager leaves each route table exactly as independent as it was before.

## Transitive routing, and what is not transitive

Most hybrid mistakes are transitivity mistakes, so hold the rule in one place. A Transit Gateway is transitive by design: attachments reach each other through its route tables, which is the whole reason it replaces a peering mesh. VPC peering is not transitive. If VPC A peers with B and B peers with C, A cannot reach C through B, and no route table entry creates that path; peering also rejects overlapping CIDRs and does not carry traffic to an on-premises network reached through the peer. Transit Gateway peering behaves the same way at the gateway level: two peered gateways exchange traffic only for prefixes with explicit static routes, and a third gateway peered to one of them is not reachable through it. A virtual private gateway is not a transit device either, which is why a VGW cannot forward between a VPN and a Direct Connect VIF. PrivateLink is deliberately not transitive at all: it exposes one service endpoint, not a network, which is exactly why it scales across thousands of consumers without any CIDR coordination.
...
- [Direct Connect virtual interfaces](https://docs.aws.amazon.com/directconnect/latest/UserGuide/WorkingWithVirtualInterfaces.html): private, public and transit destinations, BGP, and the table of which VIF types support SiteLink
...
- [Site-to-Site VPN Concentrator](https://docs.aws.amazon.com/vpn/latest/s2svpn/vpn-concentrator.html): 25 or more sites, 100 Mbps per site, 5 Gbps aggregate, Transit Gateway and BGP only
